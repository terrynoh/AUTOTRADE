# CLAUDE.md — AUTOTRADE (v3.3, R-12 구현 완료)
...
>
> **버전**: v3.3 (2026-04-15, R-12 리스크 관리 강화 구현 완료)
> **이전 버전**: v3.2 (2026-04-15, R-11), v3.1 (2026-04-10, R-08), v3 (2026-04-10), v2 (2026-04-09, R-07), v1 (프로젝트 시작)
> **현재 단계**: R-12 구현 완료, DRY_RUN 운영 중
...
### 매수가 (R-11 Double/Single 분기, yaml 동결값)

프로그램 순매수 비중에 따라 Double(≥10%) / Single(<10%) 분기:

| 시장 | 비중 | 1차 매수가 | 2차 매수가 | 간격 | 손절 |
|---|---|---|---|---|---|
| KOSPI | Double (≥10%) | 고가 -1.9% | 고가 -2.4% | 0.5% | -3.0% |
| KOSPI | Single (<10%) | 고가 -2.5% | 고가 -3.5% | 1.0% | -4.0% |
| KOSDAQ | Double (≥10%) | 고가 -2.9% | 고가 -3.9% | 1.0% | -4.4% |
| KOSDAQ | Single (<10%) | — | — | — | **매매 제외** |

**R-11 의도** (수석님 확정 2026-04-15): 프로그램 순매수 비중 ≥10% = 외인/기관 지지력 강함 → 얕은 눌림에서 진입 (Double). 비중 <10% = 지지력 약함 → 더 깊은 눌림 대기 (Single). KOSDAQ Single은 지지력 불충분으로 매매 제외.
...
| 조건 | 임계값 |
|---|---|
| 하드 손절 | R-11 Double/Single별: KOSPI Double -3.0% / KOSPI Single -4.0% / KOSDAQ Double -4.4% (호가 단위 ceil) |
...
- **kospi_double_hard_stop_pct**: 3.0 (R-11)
- **kospi_single_hard_stop_pct**: 4.0 (R-11)
- **kosdaq_double_hard_stop_pct**: 4.4 (R-11, KOSDAQ Single은 매매 제외)
...
| `risk` | daily_loss_limit_pct, max_position_size_pct, index_drop_halt_pct, max_hard_stops_daily |
...
- **책임**: 손절가 계산, 일일 손실 한도, 포지션 가용 확인, 지수 급락/손절 횟수 기반 매매 중단
- **기존 메서드**: `calculate_available_cash`, `check_daily_loss_limit`, `can_open_position`, `record_trade_result`, `reset_daily`
- **R-12 신규 메서드**:
  - `update_futures_price(price)` — 선물 가격 수신 시 고점 갱신 + 하락 체크 (당일 고점 대비 1.5% 하락 시 매매 중단)
  - `record_hard_stop()` — 하드 손절 발생 시 횟수 증가 + 한도 체크 (2회 도달 시 매매 중단)
- **R-12 신규 필드**: `_futures_high` (당일 선물 고점), `_hard_stop_count` (손절 횟수)
...
## 부록 A-2 — R-11 구현 작업 요약 (2026-04-15)

### 배경
프로그램 순매수 비중에 따라 매매 파라미터를 분기하는 R-11 redesign.
- Double (≥10%): 외인/기관 지지력 강함 → 얕은 눌림 진입
- Single (<10%): 지지력 약함 → 깊은 눌림 대기
- KOSDAQ Single: 매매 제외 (지지력 불충분)

### 진단 (M-22)
| 항목 | 진단 결과 |
|---|---|
| Watcher 필드 | `program_net_buy_ratio` 필드 없음 → `is_double_digit: bool` 추가 |
| `_fire_trigger` | market만으로 분기 → market + is_double_digit 4→3분기 |
| `start_screening` | 비중 미전달 → cand.program_net_buy_ratio 읽어서 전달 |
| Screener | KOSDAQ Single 제외 로직 없음 → 필터 추가 |

### 작업 (W-21 ~ W-24)

| # | 작업 | 영역 | 결과 |
|---|---|---|---|
| W-21 | yaml + settings.py 파라미터 구조 변경 | config/ | program_net_buy_ratio_double, kospi_double/single, kosdaq_double 파라미터 추가 |
| W-22 | Watcher 필드 + _fire_trigger 분기 | watcher.py | is_double_digit 필드, 3분기 로직 |
| W-23 | start_screening 비중 전달 | watcher.py | ratio >= threshold → is_double 플래그 |
| W-24 | Screener KOSDAQ Single 제외 | screener.py | KOSDAQ + ratio < 10% → 제외 |

### 운영 배포 (2026-04-15)

**배포 대상 4 파일**:
1. `config/strategy_params.yaml` (W-21)
2. `config/settings.py` (W-21)
3. `src/core/screener.py` (W-24)
4. `src/core/watcher.py` (W-22 + W-23)

**검증 결과**:
- py_compile 4 파일 ✅
- yaml 파싱 + 값 검증 ✅
- KOSDAQ Single 제외 로직 동작 확인 ✅

**가동 상태**: 12:31 / 13:31 KST 재시작 완료, DRY_RUN 모드
...
## 부록 A-3 — R-12 구현 작업 요약 (2026-04-15)

### 배경
수석님 요청: 리스크 관리 강화 — 지수 급락/손절 횟수 기반 매매 중단
- 지수(선물) 당일 고점 대비 1.5% 하락 시 매매 중단
- 일일 손절 2회 도달 시 매매 중단
...
| # | 작업 | 영역 | 결과 |
|---|---|---|---|
| W-25 | yaml 파라미터 추가 | strategy_params.yaml | index_drop_halt_pct: 1.5, max_hard_stops_daily: 2 |
| W-26 | settings.py 필드 추가 | settings.py | RiskParams 에 2개 필드 추가 |
| W-27 | risk_manager.py 로직 추가 | risk_manager.py | update_futures_price, record_hard_stop 메서드 |
| W-28 | main.py 연동 | main.py | _on_futures_price, _on_exit_done 에서 RiskManager 호출 |

### 수정 파일 (4개)

1. `config/strategy_params.yaml` — risk 섹션 파라미터 2개 추가
2. `config/settings.py` — RiskParams 필드 2개 추가
3. `src/core/risk_manager.py` — 지수 급락/손절 횟수 추적 로직 추가
4. `src/main.py` — RiskManager 연동 (선물 가격 전달 + 손절 횟수 기록)

### 신규 yaml 파라미터

```yaml
risk:
  daily_loss_limit_pct: 3.0           # 기존: 일일 손실 한도 (예수금 대비 %)
  max_position_size_pct: 100          # 기존: 최대 포지션 크기 (예수금 대비 %)
  index_drop_halt_pct: 1.5            # R-12 신규: 지수(선물) 당일 고점 대비 N% 하락 시 매매 중단
  max_hard_stops_daily: 2             # R-12 신규: 일일 손절 횟수 한도 (도달 시 매매 중단)
```

### 성격 분류 (기존 청산 조건 ⑤ vs R-12 신규)

| 구분 | 기존 청산 조건 ⑤ (Watcher) | R-12 신규 (RiskManager) |
|------|------------------------|-------------------------|
| 기준 | 종목 트리거 시점 선물가 | 당일 선물 고점 |
| 레벨 | 종목 단위 | **전역 단위** |
| 효과 | 해당 종목만 청산 | **신규 매수 차단** |
| 위치 | Watcher._check_futures_drop | RiskManager.update_futures_price |

→ 성격이 다름. 분리 유지가 맞음.

---

**문서 끝 (v3.3, 2026-04-15)**
