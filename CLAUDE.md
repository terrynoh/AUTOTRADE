...
| 조건 | 내용 | yaml 키 |
|------|------|---------|
| 조건 1 | 거래대금 ≥ 500억 원 | `screening.volume_min` |
| 조건 2 | 프로그램순매수비중 ≥ 5% | `screening.program_net_buy_ratio_min` |
| 조건 3 | 주가상승률 최고 1종목 | `screening.top_n_gainers` |

```
프로그램순매수비중(%) = (프로그램 순매수금액 / 9:50까지 누적 거래대금) × 100
```
...
| KOSDAQ | 고가 대비 -3.75% | 고가 대비 -4.25% | `entry.kosdaq_buy1_pct`, `entry.kosdaq_buy2_pct` |
...
```
① 하드 손절   : 고가 대비 KOSPI -4.5% / KOSDAQ -6.5% → 즉시 시장가
② 추세 이탈   : 눌림 최저가 이후 1분봉 higher lows 깨짐 → 전량 청산
③ 25분 타임아웃: 눌림 최저가 시점부터 25분, 최저가 갱신 시 리셋
...
| 09:50 스크리닝 | KIS REST API | 거래대금, 프로그램매매, 상승률 |
...
**다음 단계**: Phase 0 백테스트 실행 (pykrx 버그 우회 방법 검토 중)
...
- **선물 데이터**: KOSPI200 선물(101S) WebSocket 구독 필요 (손절 조건 ⑤)
