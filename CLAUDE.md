...
>
> **버전**: vLive 2.6.1 (2026-04-22 W-30 검증 후속 정리 + W-31 진행 중 프로젝트 인덱싱)
> **현재 단계**: R16 Phase 1 + Phase 2 B + W-SAFETY-1 + M-LIVE-02-FIX + M-LIVE-08-FIX + W-LIVE-09/10 + R-12 재설계 완료 → Day 4 (2026-04-22 예정) 체결통보 + race fix + orphan safety net + strict/halt 실전 검증
...
### 스크리닝 조건 (2026-04-22 Day 4 실거래 로그 기반 명시)

09:31/09:40/09:42/09:49 스크리닝에서 순차 필터로 Watcher 생성 여부 결정.

| 순서 | 조건 | 미달 시 | 근거 |
|------|------|---------|------|
| 1 | 수동 입력 종목 (대시보드) | 대상 아님 | `main.set_manual_codes` |
| 2 | **거래대금 ≥ 500억** | 제외 | `screener.py::run_manual L122` — `제외(거래대금 부족): ... 476억 < 500억` |
| 3 | **프로그램 순매수 비중 ≥ 5%** | 제외 | `screener.py::run_manual L174` — summary 로그 `프로그램순매수비중 5.0% 이상: N종목` |
| 4 | 시장 × 비중 분류 | — | §2 매수가 표 |
| | ├─ KOSPI + 비중 ≥ 10% | Double 진입 | |
| | ├─ KOSPI + 비중 5~10% | Single 진입 | |
| | ├─ KOSDAQ + 비중 ≥ 10% | Double 진입 | |
| | └─ KOSDAQ + 비중 5~10% | **매매 제외** | §2 매수가 표 KOSDAQ Single 행 |

**실측 예시 (2026-04-22 에이피알 278470)**:
- 09:31:45 첫 스크리닝: 거래대금 476억 < 500억 → 2단계 컷 → 제외
- 09:40:15 재스크리닝: 거래대금 536억 통과 + 비중 33.76% → Double 판정 → `[Coordinator] Watcher 가동: 에이피알(278470) KOSPI 비중=33.8% (Double)`

**보조 파라미터 (yaml 확정 위치 확인 필요)**:
- `screener.min_trading_value_krw`: 500억 (추정 키명)
- `screener.min_program_net_buy_ratio_pct`: 5.0 (추정 키명)
- `program_net_buy_ratio_double`: 10.0 (확정, §2 주요 파라미터 참조)
...
| 조건 | 임계값 | 효과 |
|------|--------|------|
| 하드손절 1회 청산 | 당일 한정 | strict 모드 진입 (KOSPI 비중 ≥18% 만 매수) |
| 하드손절 2회 청산 | 당일 한정 | 매매 전면 중단 (trading_halted) |
| 일일 손실 | 예수금 대비 3% | 매매 전면 중단 |
...
**리스크 관리** (R-12, 2026-04-22 재설계):
- `daily_loss_limit_pct`: 3.0
- `max_hard_stops_daily`: 2 (1회=strict, 2회=halt)
- `strict_mode_program_ratio_threshold`: 18.0
- `strict_mode_market_whitelist`: ["KOSPI"]
...
| RiskManager | risk_manager.py | 일일 손실, 하드손절 strict/halt 2단계 추적 |
...
- vault: `C:\Users\terryn\AUTOTRADE\Obsidian`
...
| ID | 목적 | 상태 | 경로 |
|---|------|------|------|
| W-30 / ext / ext2 | 2026-04-22 4종목 미매매 원인 진단 (KIS REST 분봉 UN/J 비대칭 확인, H1 기각, H-WS 불확정) | ✅ 완료 (임시 자원 삭제) | 핵심 숫자 W-31 §2 보존 |
| W-31 | WebSocket `H0UNCNT0` KRX-only 종목 수신 검증 (2026-04-23 로깅 디자인 예정, 매매 로직 미변경 · 관측 전용) | ⏳ 진행 중 | `Obsidian/W-31_WebSocket_런타임로깅_검증.md` |
...
### vault 위치
`C:\Users\terryn\AUTOTRADE\Obsidian`
...
- ISSUE 번호 체계 전수 목록
- **W-31 WebSocket 런타임 로깅 검증** (소규모 검증 프로젝트, 2026-04-22 작성 · 2026-04-23 실행 예정)

---

**문서 끝 (vLive 2.6.1, 2026-04-22 W-30 검증 후속 정리 + W-31 진행 중 프로젝트 인덱싱)**
...
- **vLive 2.5 (2026-04-22)**: §2 스크리닝 조건 파이프라인 명시.
  - §2 시각 트리거와 매수가 사이에 **스크리닝 조건** 섹션 신설. 거래대금 ≥ 500억 (2단계) + 프로그램 순매수 비중 ≥ 5% (3단계) 사전 필터를 명문화. 기존 §2 매수가 표의 Double/Single 경계(10%)는 4단계 분류로 재배치.
  - 근거: 2026-04-22 Day 4 실거래 로그 `screener.py::run_manual L122/L174`. 에이피알(278470) 09:31 거래대금 476억 컷 → 09:40 536억 통과 실측 확정.
  - yaml 파라미터 키명 2건은 추정 상태로 명시 (`min_trading_value_krw`, `min_program_net_buy_ratio_pct`). 실제 yaml 확인 후 확정 갱신 예정.
- **vLive 2.6 (2026-04-22)**: R-12 재설계. 지수급락 -1.5% halt 폐기. 하드손절 의미 계층화 (1회=strict_mode / 2회=trading_halted). strict_mode 조건 KOSPI AND program_ratio ≥18%, 당일 한정 (reset_daily 복귀). 수정 파일 7개 (코드 5 + 문서 2: CLAUDE.md, README.md).
- **vLive 2.6.1 (2026-04-22)**: W-30 검증 후속 정리 + W-31 진행 중 프로젝트 인덱싱. §8 "진행 중 검증 프로젝트" 서브섹션 신설 (W-30 완료 + W-31 착수 예정). §9 vault 주요 문서 목록에 W-31 1줄 추가. 본 CLAUDE.md 는 포인터만 유지, 상세 내용은 Obsidian 별도 문서 (`W-31_WebSocket_런타임로깅_검증.md`).
